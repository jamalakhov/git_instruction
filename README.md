...
1. **git reset**
    ...
    Данная команда сбрасывает состояние нашего репозитория до определенного коммита и **удаляет** коммиты из истории которые следуют после того комиита, хэш которого мы указали в команде.

    В результате выполнения данной команды, коммит, хэш которого мы указали, пропадет из истории коммитов, но из самого **Git** он не исчезнет. Чтобы посмотреть **расширенную** версию истории выполним команду
